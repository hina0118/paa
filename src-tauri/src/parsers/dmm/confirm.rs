...
pub(crate) fn extract_items_from_html(document: &Html) -> Result<Vec<OrderItem>, String> {
    ...
    // 重複排除: (name, unit_price, quantity) が同一のアイテムをマージする。
    // DMM確認メールは商品ごとに画像アンカーとテキストアンカーの両方が
    // `dmmref=gMono_Mail_Purchase` を持つ構造のため、同一商品が2件 push される。
    // どちらかにしかない Optional フィールドを統合し、情報量の多い1件に集約する。
    let mut deduped: Vec<OrderItem> = Vec::new();
    for item in items {
...
<td>800円</td>
    ...
    /// 実際のDMMメール構造: 商品ごとに画像アンカーとテキストアンカーの2つが存在する場合、
    /// 1件に重複排除され、image_url はどちらかに存在すれば採用される
    #[test]
    fn test_parse_dmm_confirm_html_dual_anchor_dedup() {
        // 実際のDMMメールに近い構造:
        //   <td width="120"><a href="...?dmmref=gMono_Mail_Purchase"><img src="pics.dmm.com/..." alt="商品名"></a></td>
        //   <td><a href="...?dmmref=gMono_Mail_Purchase">商品名テキスト</a>  価格・数量</td>
        let html = r#"<html><body>
<table>
<tr>
<td>BS-28972148</td>
<td>発送元：千葉配送センター</td>
<td>発送：配送業者は発送時に確定</td>
</tr>
</table>
<table>
<tr>
<td>ご注文日：2026/2/25</td>
</tr>
<tr>
<td>受取人のお名前：テスト 太郎 様</td>
</tr>
</table>
...
<a href="https://www.dmm.com/mono/hobby/-/detail/=/cid=c250901396/?dmmref=gMono_Mail_Purchase">
<img src="https://pics.dmm.com/mono/hobby/c250901396/c250901396ps.jpg" alt="30MM VP-40S ロックスミス" width="120">
</a>
</td>
<td valign="top">
<table>
<tbody>
<tr><td>発売日：2026年3月発売予定</td></tr>
<tr>
<td>
<a href="https://www.dmm.com/mono/hobby/-/detail/=/cid=c250901396/?dmmref=gMono_Mail_Purchase" style="color:#005fc0;">30MM VP-40S ロックスミス</a>
</td>
</tr>
<tr>
<td>
<table><tbody><tr>
<td style="color:#c00000;">4,180円</td>
<td style="color:#777;">数量：1</td>
</tr></tbody></table>
</td>
</tr>
</tbody>
...
<table>
<tr><td>商品小計：4,180円</td></tr>
<tr><td>送料：0円</td></tr>
<tr><td>お支払い金額：<span>4,180円(税込)</span></td></tr>
</table>
</body></html>"#;
        let parser = DmmConfirmParser;
        let result = parser.parse(html);
        ...
        assert_eq!(order_info.order_number, "BS-28972148");
        // 画像アンカーとテキストアンカーの重複が排除され、1件のみ
        assert_eq!(order_info.items.len(), 1, "Duplicate items should be merged into one");
        assert_eq!(order_info.items[0].name, "30MM VP-40S ロックスミス");
        assert_eq!(order_info.items[0].unit_price, 4180);
        assert_eq!(order_info.items[0].quantity, 1);
        // image_url は画像アンカー由来の値が採用される
        assert_eq!(
            order_info.items[0].image_url,
            Some("https://pics.dmm.com/mono/hobby/c250901396/c250901396ps.jpg".to_string())
        );
    }
}
