...
| 領域 | カバレッジ | 目標 | 状態 | テスト数 |
|------|-----------|------|------|---------|
| **バックエンド（Rust）** | 57.13% | 85% | 🔄 改善中 | 67件 |
| **フロントエンド（React）** | 93.60% | 85% | ✅ 目標達成! | 122件 |
| **全体（推定）** | ~72-74% | 85% | 🔄 順調 | 189件 |
...
| **リージョンカバレッジ** | 56.14% | 1931リージョン中847未カバー |

### 改善履歴（2026-01-17）

- 行カバレッジ: 36.13% → 53.11% → **57.13%** (+21.00%)
- 関数カバレッジ: 34.65% → 54.81% → **59.46%** (+24.81%)
- リージョンカバレッジ: 32.11% → 51.48% → **56.14%** (+24.03%)
- テスト数: 17件 → 54件 → **67件** (+50件)

### ファイル別カバレッジ

- **gmail.rs**: 66.94% (行カバレッジ) - コア機能
