...
2. **元注文の商品を削除**（apply_change_items 相当の処理）
   ...
   - 残り商品数が 0 になった注文は deliveries のみクリーンアップ。orders/order_emails は再パース防止のため保持
...
| 1.3 | 空注文のクリーンアップ            | `repository.rs` | 全商品削除後に items が 0 件になった order の deliveries のみ削除。orders/order_emails は再パース防止のため保持 |
