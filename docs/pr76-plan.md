...
**未解決レビュー**: 3件 → 対応済み（下記「7. 未解決レビュー」参照）
...
## 7. 未解決レビュー（2026-02-08 対応済み）

以下の 3 件の Copilot 指摘に対応した。

| #   | ファイル                   | 指摘                                                                                                         | 対応内容                                                                                                                                                                                                 |
| --- | -------------------------- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `email_parse_task.rs` L376 | `apply_cancel` 失敗を `Err` にすると `failed_count` に加算され、リトライ前提のケースと恒久失敗が区別できない | Task H 対応済み。設計書 6.3 の記載を明確にし、PR コメントで Copilot に「次回 run で再試行される」旨を返信することを推奨                                                                                  |
| 2   | `parsers/mod.rs` L476      | `apply_cancel` 失敗時も `cancel_applied = true` で `continue` している（実際は未適用）                       | **対応済み**: `cancel_applied` を `handled_as_cancel` にリネームし、「キャンセルメールとして処理した（成功・失敗いずれも）」のコメントを追加。変数名は「適用済み」ではなく「処理済み」を指すようになった |
| 3   | `repository.rs` L682       | `COALESCE` はインデックス `(order_number, shop_domain)` を効かせにくい                                       | **対応済み**: `shop_domain` でクエリを分岐。`shop_domain` が Some(非空) のときは `shop_domain = ?`、None/空のときは `shop_domain IS NULL OR ''` に限定する形に変更                                       |
